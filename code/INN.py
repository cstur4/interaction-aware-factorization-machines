...
#                 print(self.element_wise_product)
                ...
                self.attention_interaction = tf.reshape(self.attention_interaction, [num_interactions * self.hidden_factor[1]])
                print(str(self.AFM) + "\t" + str(self.attention_interaction) )
                ...
                self.AFM = tf.layers.dense(hidden1, 1)
                #self.AFM = tf.squeeze(self.AFM, -1)
                print(self.AFM)
                #self.AFM = tf.reshape(self.AFM, [-1, ], name="afm")# None * M" * K
                #self.AFM = tf.reduce_sum(tf.multiply(self.attention_out, self.element_wise_product), 1, name="afm") # None * K
            else:
                self.AFM = tf.multiply(self.element_wise_product, 1, name="afm") # None * M" * K
                #self.AFM_FM = tf.reduce_sum(self.element_wise_product, 1, name="afm_fm") # None * K
            #self.AFM_FM = self.AFM_FM / num_interactions
            
            print("attention = " + str(self.attention_out))
            print("element_wise = " + str(self.element_wise_product))
            print("afm = " + str(self.AFM))
    ...
    def _initialize_weights(self):
        ...
        if self.pretrain_flag > 0 or self.micro_level_analysis:
            ...
            # if self.micro_level_analysis:
            from_file = self.save_file.replace('tafm_', '')
        ...
        # attention
        interaction_factor_hidden = self.kf#16
        if self.attention:
        ...
        # prediction layer
        all_weights['prediction'] = tf.Variable(np.ones(((self.hidden_factor[1] * self.valid_dimension * (self.valid_dimension-1)) // 2, 1), dtype=np.float32))  # hidden_factor * 1
...
def train(args):
    ...
    if args.verbose > 0:
        print(("AFM: dataset=%s, factors=%s, attention=%d, freeze_fm=%d, #epoch=%d, batch=%d, lr=%.4f, lambda_attention=%.1e, lambda_attention1=%.1e, kf=%d, temp=%.1e, keep=%s, optimizer=%s, batch_norm=%d, decay=%f, activation=%s"
